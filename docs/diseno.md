...
**Cálido y orgánico.** Fondo de arena tibia, fichas blancas que se apoyan
encima, esquinas redondas y un color de marca de barro. Elegante, pero de app
— no de documento.
...
1. **El fondo NO es blanco; las fichas SÍ.** Ese par de tonos de diferencia
   (`--color-bg` arena contra `--color-surface` blanco) es lo que hace que una
   ficha flote. La sombra solo lo termina — si se quita el contraste de fondo y
   se compensa con más sombra, se ve sucio.
...
3. **Las sombras son cálidas, nunca grises.** `rgba(78,58,40,…)`, en dos capas:
   una pegada y una difusa. Una sombra neutra sobre arena se ve como mugre.

4. **Un solo color de marca.** El barro (`--color-accent`) es para acciones,
...
Dos colores de tierra, no de semáforo:

| | Light | Dark | Qué es |
|---|---|---|---|
| `.positivo` | `#48784f` salvia | `#84b78c` | lo que entra |
| `.negativo` | `#8a3246` vino | `#dd8398` | lo que sale |
| `.advertencia` | barro | barro | **lo que pide una acción** |
| `.neutro` | gris | gris | un traslado, que no suma ni resta |

El vino tira a morado y el barro a naranja, así que **no se confunden aunque
caigan en la misma fila** — eso fue deliberado al elegirlos.
...
Y ojo: el barro **no es "positivo" ni "negativo"**. Es "atiéndeme" — una deuda
con saldo, una cuota vencida, un cobro de hoy.
...
De noche el fondo es un **carbón cálido**, no negro, y las fichas quedan un par
de tonos por encima para que sigan flotando. Los tres colores (barro, salvia,
vino) suben de luminosidad: los de día, sobre carbón, se hunden. La rampa del
acento se **invierte** — el paso que en claro es el oscuro pasa a ser el claro.
...
3. **Versión actual.** Se conservó la organización de las pantallas —que era lo
   bueno del rediseño— y se cambió la piel: más volumen, tipografía menos
   solemne y una paleta de tierra.
