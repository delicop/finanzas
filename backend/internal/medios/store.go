...
func (s *Store) Listar(ctx context.Context, usuarioID int64) ([]Medio, error) {
	const q = `
		SELECT c.id, c.nombre, c.creado_en, c.actualizado_en,
		       count(m.id) AS movimientos
		FROM medios_pago c
		LEFT JOIN movimientos m ON m.medio_pago_id = c.id
...
func (s *Store) PorID(ctx context.Context, usuarioID, id int64) (*Medio, error) {
	const q = `
	...
	if err != nil {
		return nil, fmt.Errorf("consultando medio: %w", err)
	}
	return &c, nil
}

func (s *Store) Crear(ctx context.Context, usuarioID int64, nombre string) (*Medio, error) {
	const q = `
	...
	if err != nil {
		...
		return nil, fmt.Errorf("creando medio: %w", err)
	}
	return &c, nil
}

func (s *Store) Actualizar(ctx context.Context, usuarioID, id int64, nombre string) (*Medio, error) {
	const q = `
		UPDATE medios_pago
